  ...
  app.post('/api/match', apiLimiter, async (req, res) => {
    try {
      ...
      if (!parsedData.success) {
        return res.status(400).json({ error: 'Invalid payload structure', details: parsedData.error.issues });
      }
      ...
      res.status(500).json({ error: 'Failed to process match recommendations. Please try again later.' });
    }
  });
