            ...
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <div className="flex items-center gap-3">
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-bold text-sky-900 leading-tight">{profile?.displayName || 'Client'}</p>
                  <p className="text-[10px] font-medium text-slate-500 uppercase tracking-tighter">Clinical Partner</p>
                </div>
                <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-primary-container/20">
                  <img src={profile?.photoURL || `https://api.dicebear.com/7.x/initials/svg?seed=${profile?.displayName || 'C'}`} alt="avatar" className="w-full h-full object-cover" />
                </div>
              </div>
            </div>
          </div>
